...
% Last Modified by GUIDE v2.5 09-Jun-2016 15:12:28
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global getOUT; global tiffOUT; global ALL;
hold on;
masterData = transfer(getOUT);
TIFF = transfer(tiffOUT);
frame = str2double(get(handles.frame,'String'));                      
%TIFF = padarray(TIFF, [20,20,0]);
%displayGraph(TIFF(:,:,frame),masterData(frame).VALL, ...
%    masterData(frame).EALL,'on');
